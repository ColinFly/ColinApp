...
/**
 * 
 * <p><B>类名：</B><br/>StringUtil</p><br/>
 * <p><B>描述：</B><br/>字符串操作工具类  </p><br/>
 * <p><B>作者：</B><br/> ~若相惜</p><br/>
 * <p><B>@date</B><br/> 2015-4-3 下午5:44:33</p>
 *
 */
public class StringUtil {
	...
	/*public static String friendly_time(String sdate) {
		...
		if(curDate.equals(paramDate)){
			...
			if(hour == 0)
				//服务器访问太慢，约为2分钟 ;本地服务器，是1分钟。人为减一。
				ftime = Math.max((cal.getTimeInMillis() - time.getTime()) / 60000 - 1,1)+CAppContext.appContext().context().getString(R.string.cb_time_minute);
			else 
				ftime = hour+CAppContext.appContext().context().getString(R.string.cb_time_hour);
			return ftime;
		}
		...
		if(days == 0){
			...
			ftime = CAppContext.appContext().context().getString(R.string.cb_time_yesterday);
		}
		else if(days == 2){
			ftime = CAppContext.appContext().context().getString(R.string.cb_time_before_yesterday);
		}
		else if(days > 2 && days <= 10){ 
			ftime = days+ CAppContext.appContext().context().getString(R.string.cb_time_day);
		}
		else if(days > 10){			
	...
	private StringUtil(){} // don't instantiate
